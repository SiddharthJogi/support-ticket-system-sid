    ...
    <div className="min-h-screen bg-[#FAFAFA] font-sans selection:bg-sud-red selection:text-white">
      ...
      <div className="max-w-5xl mx-auto px-6 pb-20">
         <div className="flex justify-between items-end mb-8">
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowModal(true)} className="flex items-center gap-2 bg-sud-blue text-white px-6 py-3 rounded-xl font-semibold shadow-lg shadow-blue-900/20 hover:bg-blue-800 transition"><FiPlus /> New Ticket</motion.button>
         </div>
         ...
         {tickets.length === 0 ? (
            ...
            <div className="grid gap-4">
                {tickets.map((t, i) => (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} key={t.ticket_id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center hover:shadow-md transition">
                        <div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
           ...
           <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-3xl p-8 w-full max-w-lg relative z-10 shadow-2xl">
...
const ActionCard = ({ title, icon, delay, active }) => (
  <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay }} whileHover={{ y: -5 }} className={`p-6 rounded-2xl cursor-pointer border transition-all ${active ? 'bg-sud-blue text-white shadow-xl shadow-blue-900/20 border-transparent' : 'bg-white text-gray-600 border-gray-100 hover:shadow-lg hover:border-blue-100'}`}>
